...
function* onChatRequestSend(actions) {
    ...
    try {
        ...
        console.log('register_rsponse ::: ', profileId,imagePalmistry?.imagePick);

        if(imagePalmistry?.imagePick == 'Yes') {

            const response = yield RNFetchBlob.fetch(
                'POST',
                api_url + palmistry_image,
                {
                    'Content-Type': 'multipart/form-data',
                },
                [
                    { name: 'linkedProfileId', data: profileId },
                    {
                    name: 'image',
                    filename: `${customerData?._id}.jpg`,
                    type: 'image/jpeg', // Specify the correct MIME type
                    data: RNFetchBlob.wrap(imagePalmistry?.imageHand), // Wrap file path
                    },
                ]
                );
        ...
        yield put({ type: actionTypes.SET_CAMERA_CHAT_HAND, payload: null});
        ...
        if (type == 'chat') {
            ...
            console.log('send request ::: ',send_request);
            if (send_request.success) {
                ...
                socketServices.emit('createChatRoom', {
                    ...
                    newUser: customerData?.free_chat == 1 ? false : true,
                });
            ...
            if (customerData?.wallet_balance < chatPrice * 5) {
                ...
                // Zego cloud internet calling
                yield put({ type: actionTypes.ON_CALL_TO_ASTROLOGER, payload: { customerId: customerData?._id, astrologerId: astrologerId, formId: profileId, astrologerName, navigation, chatPrice } })
...
function* getOnViedoCallEnd(actions) {
    try {
        const { payload } = actions
        const generateIdData = yield select(state => state.chat.generateIdData)
        console.log(generateIdData?.type, 'checkData')
        console.log("call Id Payload data :::", payload);
        const data = {
        ...
        if (response?.data.success) {
            yield put({ type: actionTypes.SET_GENERATEID_DATA, payload: null })
            ...
            showToastMessage({ message: `${generateIdData?.type} Ended Sucessfully` });
...
function* getOnAudioCallEnd(actions) {
    try {
        const { payload } = actions
        ...
        const response = yield axios.post(api_url + endAudiocall, data);
        console.log(response?.data, 'Audiodata')
        if (response?.data.success) {
            // yield put({ type: actionTypes.SET_VIDEO_INVOICE_DATA, payload: response?.data?.data })
            // yield put({ type: actionTypes.SET_VIDEOCALL_INVOICE_VISIBLE, payload: true })
            showToastMessage({ message: 'Audio Ended' });

            console.log(' end chshsh')
        }
        resetToScreen('home')
    } catch (e) {
        console.log(e, 'error Audio');
    }
}
...
export default function* chatSaga() {
    ...
    yield takeLeading(actionTypes.GET_ON_AUDIO_CALL_END, getOnAudioCallEnd)
