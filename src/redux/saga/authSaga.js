...
import { registerZegoAudioCall } from '../../utils/zegoServicesAudio';
...
function* onLogin(actions) {
    try {
        ...
        if (response?.status) {
            ...
            // showToastMessage({ message: `Your OTP is: ${response?.otp}` });
        } else {
...
//             // yield registerZegoAudioCall({userId:response?.customer?._id, userName: response?.customer?.customerName || 'Unknown', dispatch: null})
//         }else{
//             showToastMessage({ message: response?.message })
//         }
...
function* onGoogleLogin(actions) {
    ...
    try {
        ...
        if (!userInfo || !userInfo.data || !userInfo.data.user) {
            throw new Error('User information is undefined or incomplete');
        }
        ...
        const response = yield postRequest({
            ...
            data: {
                ...
                fcmToken: yield getFcmToken(),
                device_id: 'device_id',
                customerName: userInfo.data.user.name,
            },
        });
        ...
        showToastMessage({ message: 'Login failed. Please try again.' });
    } finally {
        yield put({ type: actionTypes.SET_IS_LOADING, payload: false });
    }
}
...
function* onOtpVerification(actions) {
    try {
        ...
        console.log(response, "shivam data")
        if (response?.success) {
            yield AsyncStorage.setItem('customerData', JSON.stringify(response?.customer))
            yield put({ type: actionTypes.SET_CUSTOMER_DATA, payload: response?.customer })
            ...
            if (response?.type == 'home') {
                ...
                // yield registerZegoAudioCall({userId:response?.customer?._id, userName: response?.customer?.customerName || 'Unknown', dispatch: payload?.dispatch})
            } else {
...
function* onRegister(actions) {
    try {
        ...
        if (response?.success) {
            ...
            // yield registerZegoAudioCall({userId:response?.data?._id, userName: response?.data?.customerName || 'Unknown', dispatch: payload?.dispatch})
        } else {
