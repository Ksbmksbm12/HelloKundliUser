...
function* getSplash(actions) {
    try {
        ...
        const dataCall = yield select(state => state.chat.astrologerCallData);
        ...
        if (custData) {
            ...
            if (response?.success) {
                ...
                // yield registerZegoAudioCall({userId: response?.customersDetail?._id, userName: response?.customersDetail?.customerName || 'Unknown', dispatch: payload})
            }else{
