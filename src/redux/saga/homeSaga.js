...
import { api_url, customer_home_banner, get_active_astrologer, get_astro_companion, get_call_astrologer, get_chat_astrologer, get_delete_account_data, get_notification_data, get_video_call_astrologer, update_customer_notification } from '../../config/constants';
...
function* getNotificationData(actions) {
    try {
        ...
        if (response?.success) {
            yield put({ type: actionTypes.SET_NOTIFICATION_DATA, payload: response?.data })
            yield put({ type: actionTypes.SET_NOTIFICATION_COUNTS, payload: response?.data })
        }
...
function* updateNotfication(actions) {
    try {
        const { payload } = actions;
        console.log('Payload :::', payload);
        const response = yield axios.post(api_url + update_customer_notification, payload?.data);
        console.log("response notification :::", response?.data);
        if (response?.data?.success) {
            showToastMessage({ message: "notification details" });

            const responseNotification = yield postRequest({
                url: api_url + get_notification_data,
                data: {
            ...
            yield put({ type: actionTypes.SET_NOTIFICATION_COUNTS, payload: responseNotification?.data });
...
export default function* homeSaga() {
    ...
    yield takeLeading(actionTypes.UPDATE_CUSTOMER_NOTIFICATION, updateNotfication);
}