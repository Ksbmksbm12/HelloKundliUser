...
import { makeZegoAudioCall } from '../../utils/zegoServicesAudio';
...
function* onCallToAstrologer(actions) {
  try {
    ...
    if(response.data.success) {
      yield put({ type: actionTypes.SET_ASTROLOGER_CALL,payload: response.data.data });
      yield makeZegoCall({ isVideo: false, navigation, newInvitees, data: response.data.data })
      yield put({ type: actionTypes.SET_GENERATEID_DATA, payload: response?.data?.data })
    } else {
...
function* onVideoCallToAstrologer(actions) {
  try {
    ...
    console.log('Response Call ID ::::',response.data);
    if(response.data.success) {
      console.log('Videocall start zego')
