...
const PlaceOfBirth = ({ dispatch, navigation, route }) => {
  ...
  console.log(route.params?.title, 'check')
  useEffect(() => {
    navigation.setOptions({
      header: () => (
        <MyHeader
          // title="Place of Birth"
          title={route.params?.title === 'location' ? 'Search Location' : 'Place of Birth'}
          socialIcons={false}
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <GooglePlacesAutocomplete
        placeholder={route.params?.title === 'location' ? 'Search Location' : 'Birth Place'}
