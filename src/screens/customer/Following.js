      ...
      >
        <View style={[styles.row, { justifyContent: 'space-between' }]}>
          <View style={styles.row} >
            <View style={{width: width * 0.2, height: width * 0.2,borderRadius:50,padding:2}}>
            <Image source={{ uri: base_url + item?.profileImage }} style={{height:'100%',width:'100%',borderRadius:50}} resizeMode='conver' />

            </View>
            <View style={{ marginLeft: Sizes.fixPadding }}>
              <Text allowFontScaling={false} style={{...Fonts.JostMediumBlack,fontSize:16}}>{item.astrologerName}</Text>
              <Text allowFontScaling={false} style={{...Fonts.JostMediumBlack,color:'#4B4B4B'}}>{item.gender}</Text>
            </View>
        ...
        <TouchableOpacity activeOpacity={0.8} onPress={() => dispatch(AstrologerActions.onFollowUnfollowAstrologer(item?._id))} style={{ ...styles.actions, borderRadius: 10 }}>
          <Text allowFontScaling={false} style={{...Fonts.JostRegularBlack}}>Unfollow</Text>
        </TouchableOpacity>
