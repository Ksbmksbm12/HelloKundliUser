...
import CameraHand from '../../utils/cameraHand';
...
const ChatIntakeForm = ({
  ...
  console.log(chatCameraHand, 'adress data')
  useEffect(() => {
  ...
  return (
    <View
      ...
      {startChatModalInfo()}
      {linkedProfileInfo()}
      {cameraVisibleChat && <CameraHand />}
    </View>
  );

  function newPalamHand() {
    return (
      <View>
        <Text style={{ color: 'black' }}>Do you want to take palmistry service?</Text>
        <View
          style={{
            borderWidth: 1,
            borderColor: '#ddd',
          ...
          <Picker
            selectedValue={imageHandOption}
            themeVariant="dark"
            // enabled={!newProfile}
            onValueChange={(itemValue, itemIndex) => {
              updateState({ imageHandOption: itemValue });
            }}
        ...
        {!chatCameraHand && imageHandOption == 'Yes' && (
          <TouchableOpacity onPress={() => dispatch(ChatActions.setCameraChatVision(true))}
            style={{ backgroundColor: colors.background_theme2, padding: 10, alignSelf: 'center', borderRadius: 10, top:10,marginBottom:20 }}>
            <Text style={{ color: 'black' }}>On Camera</Text>
          </TouchableOpacity>
        )}

        {chatCameraHand && (
          <View style={{alignSelf:'center',marginTop:20}}>
            <TouchableOpacity style={{backgroundColor:colors.background_theme1,position:'absolute',zIndex:10,borderRadius:40,left:width * 0.37,top:height * -0.02,}}
            onPress={() => dispatch(ChatActions.setCameraChatHand(null))}>
              <AntDesign name='closecircle' size={30} color={'red'} />
            </TouchableOpacity>
            <Image source={{ uri: chatCameraHand }} width={150} height={150} style={{ borderRadius: 10 }} />
          </View>
  ...
  function startChatModalInfo() {
    ...
    const onStartChat = () => {
      ...
      if (route.params?.astrostatus != 'online') {
        ...
        console.log('start')
        return
      } else if (imageHandOption == 'Yes' && !chatCameraHand) {
        showToastMessage({ message: 'Please Upload Image Palmistry'});
        setModalVisible1(false);
        return
      }   
      else if (customerData?.wallet_balance < astroData?.chatPrice * 5) {
          ...
          const payload = {
            ...
            imagePalmistry: {
              imagePick: imageHandOption,
              imageHand: chatCameraHand
            },
...
const mapStateToProps = state => ({
  ...
  chatCameraHand: state.chat.chatCameraHand,
  cameraVisibleChat: state.chat.cameraVisibleChat
});
