...
const CallHistory = ({ dispatch, callHistoryData }) => {
  ...
  //   callHistoryData = 0;
  // }
  // console.log('call1',callHistoryData,'callcall')

  const click = async(id) => {
    console.log(id);
    const api = `https://api.hellokundli.in/api/customers/genrate-pdf-call-history?callHistoryId=${id}`;
  ...
  const renderItem = ({ item, index }) => {
    const commissionCallPrice = parseInt(item?.astrologerId?.commission_call_price) || 0;
    const callPrice = parseInt(item?.astrologerId?.call_price) || 0;
  
    const totalCallPrice = commissionCallPrice + callPrice;
    console.log(item,'alldatas')
    return (
      <View style={styles.container}>
        ...
        <View style={{ marginTop: Sizes.fixPadding }}>
          ...
          <Text
            style={{ ...Fonts.gray14RobotoRegular, color: Colors.blackLight }}>
            Call Price: {showNumber(totalCallPrice)}/min
          </Text>
          <Text
            style={{ ...Fonts.gray14RobotoRegular, color: Colors.blackLight }}>
