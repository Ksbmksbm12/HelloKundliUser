...
const WalletGstAmount = props => {

  const { t } = useTranslation();
  const [isLoading, setIsLoading] = useState(false);
  const [walletData] = useState(props.route.params.amount);
  ...
  const phonepe = async (amount, sub) => {
    const payload = {
      ...
      firstRechargeId: '',
      rechargePlanId: props.route?.params?.rechargePlanId ?? ''
    }
    console.log(payload, 'pay')
    props.dispatch(UserActions.onWalletRecharge(payload))
  };
