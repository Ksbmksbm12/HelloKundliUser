...
const Products = ({ navigation, route, productsData, dispatch }) => {
    ...
    function categoriesInfo() {
        const _listEmpty = () => {
            return (
                <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: SCREEN_HEIGHT * 0.4 }}>
                    <Text style={{ ...Fonts.gray14RobotoMedium, fontSize: 16 }}>No product available</Text>
                </View>
            )
        }
        ...
        return (
            <FlatList data={productsData} numColumns={2} renderItem={renderItem} ListEmptyComponent={_listEmpty} />
        )
    }
}
