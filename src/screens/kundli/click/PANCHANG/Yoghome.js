...
const Yoghome = ({ navigation, locationData, dispatch, isLoading, props }) => {
  ...
  console.log("kkkk:::SS", locationData);
  ...
  const handleFetchKundli = () => {
    if (!selectedOption) {
      warnign_toast("Please select Yog");
      return false;
      ...
      const payload = {
        ...
        month: month,
        year: year,
        lat: locationData?.lat, // You may want to replace these with actual lat and lon variables
    ...
    <View style={styles.container}>
      <View style={styles.contentContainer}>
        ...
        <TouchableOpacity style={styles.button} onPress={handleFetchKundli}>
          <Text style={{ ...Fonts.JostSemiBoldBlack, color: 'white', fontSize: 20 }}>
            Show Yog
          </Text>
        </TouchableOpacity>
      </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(Yoghome);
